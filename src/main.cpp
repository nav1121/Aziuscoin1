...
#if defined(NDEBUG)
# error "AIZEUS cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x572bf9fce12abb5e33693923de9e5c602da41a03cca98a213791aa2257c3104b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AIZEUS: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AIZEUS Signed Message:\n";
...
{
    // AIZEUS: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AIZEUS: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AIZEUS: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1528199742, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The New York Times Headline 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048d5bd8da6a9b41dad4e59cab6fde35e6d27d57faf27027dd95b7de8bc3e0e6a8646babf44adeb78c6401cfe7abd9d3d54e9e3838526aee2f067d50e78364d346") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528199742;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2186104;
        ...
        assert(block.hashMerkleRoot == uint256("0x45af33fa8ab50903b35e7043be7642ce65ef25805a33d389e43f6406479895f2"));
...
unsigned char pchMessageStart[4] = { 0xaa, 0xfc, 0x2e, 0x1e }; // AIZEUS: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AIZEUSMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AIZEUSMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AIZEUSMiner(CWallet *pwallet)
{
    printf("AIZEUSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aizeus-miner");
    ...
    try { loop {
        ...
        printf("Running AIZEUSMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AIZEUSMiner, pwallet));
}
